...
namespace ATSAPI.Controllers
{
    ...
    public class MonthStartController : ApiController
    {
        public ResponseModel GetMonthStart()
        {
            ...
            try
            {
                using (var db = new ATS2019_dbEntities())
                {
                    ...
                    List<MonthModel> offerlist = (from c in db.Offer_Master
                                                  where c.O_Off_Date.Value.Month == month && c.O_Off_Date.Value.Year == year && c.O_Status == "Join"
                                                  join
